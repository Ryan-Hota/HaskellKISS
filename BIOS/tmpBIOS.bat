echo -Wall>%HIE_BIOS_OUTPUT%

echo Link_IO.hs>>%HIE_BIOS_OUTPUT%
echo Utilities.hs>>%HIE_BIOS_OUTPUT%
echo Nub.hs>>%HIE_BIOS_OUTPUT%

echo FilePath_Internal_IO.hs>>%HIE_BIOS_OUTPUT%
echo FilePath.hs>>%HIE_BIOS_OUTPUT%
echo IO.hs>>%HIE_BIOS_OUTPUT%
echo Directory.hs>>%HIE_BIOS_OUTPUT%
echo Directory_IO.hs>>%HIE_BIOS_OUTPUT%

echo Target_IO.hs>>%HIE_BIOS_OUTPUT%
echo Options.hs>>%HIE_BIOS_OUTPUT%
echo Modules.hs>>%HIE_BIOS_OUTPUT%