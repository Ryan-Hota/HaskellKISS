...
instance Absolutable Absolute where
   toAbsolute :: AbsoluteFilePath -> AbsoluteFilePath
   toAbsolute = id
...
findRoot path =
    ...
    |> fmap (
        listToMaybe
        |> fromMaybe ( error "\nA \"hie.yaml\" file is required to be exist to define a root.\nThe given target "++show path++" is not in the scope of any defined root" )
    )

instance Absolutable RootRelative where
    toAbsolute :: RootRelativeFilePath -> AbsoluteFilePath
